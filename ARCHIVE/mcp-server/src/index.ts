...
// Circuit registry
const circuits = new Map<string, GaiaCircuit>();
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case 'create_circuit': {
      const { name: circuitName, type, config = {} } = args as any;
      
      let circuit: GaiaCircuit;
      switch (type) {
        case 'superposition':
    ...
    case 'execute_circuit': {
      const { name: circuitName, inputs } = args as any;
    ...
    case 'visualize_circuit': {
      const { name: circuitName } = args as any;
      const circuit = circuits.get(circuitName);
    ...
    case 'evolve_circuit': {
      const { name: circuitName, fitness_function, iterations = 100 } = args as any;
      const circuit = circuits.get(circuitName);
