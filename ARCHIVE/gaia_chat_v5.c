...
typedef struct {
    ...
    float probability;  // For superposition mode
} WordCandidate;

// Superposition state tracking
typedef struct {
    char words[MAX_SUPERPOSITION][MAX_WORD_LENGTH];
    float probabilities[MAX_SUPERPOSITION];
    int num_states;
    int collapse_step;
} SuperpositionState;
...
char* find_best_continuation(ChatSystem* sys, char context[][MAX_WORD_LENGTH], int context_length) {
    ...
    // Check if we should use superposition mode
    if (use_superposition && num_candidates > 1) {
        // Calculate score variance to detect ambiguity
        float avg_score = 0;
        for (int i = 0; i < num_candidates; i++) {
            avg_score += candidates[i].path_score;
        }
        avg_score /= num_candidates;
        
        // Check if top candidates are similar (ambiguous)
        int similar_count = 0;
        float top_score = candidates[best_idx].path_score;
        for (int i = 0; i < num_candidates; i++) {
        ...
        // Use superposition if multiple similar candidates
        if (similar_count > 1) {
            // Calculate probabilities
            float total_score = 0;
            for (int i = 0; i < num_candidates && i < MAX_SUPERPOSITION; i++) {
                if (candidates[i].path_score >= top_score * SUPERPOSITION_THRESHOLD) {
                    total_score += candidates[i].path_score;
                }
            }
            ...
            for (int i = 0; i < num_candidates && i < MAX_SUPERPOSITION; i++) {
                if (candidates[i].path_score >= top_score * SUPERPOSITION_THRESHOLD) {
                    candidates[i].probability = candidates[i].path_score / total_score;
                    cumulative += candidates[i].probability;
                    
                    if (random_val <= cumulative) {
                        if (debug_superposition) {
                            printf("SUPERPOSITION: Selected '%s' (prob=%.2f) from %d similar candidates\n",
                                   candidates[i].word, candidates[i].probability, similar_count);
                        }
                        static char result[MAX_WORD_LENGTH];
                        strcpy(result, candidates[i].word);
                        return result;
                    }
                }
            }
    ...
    // Default: return best candidate (deterministic)
    static char result[MAX_WORD_LENGTH];
    strcpy(result, candidates[best_idx].word);
    return result;
}

// Function pattern recognition and execution
char* try_function_call(const char* input) {
    static char function_result[MAX_FUNCTION_RESULT];
    FunctionResult result;
    
    // Convert input to lowercase for pattern matching
    char input_lower[1024];
    strncpy(input_lower, input, 1023);
    input_lower[1023] = '\0';
    ...
    // Math patterns
    if (strstr(input_lower, "what is") && strstr(input_lower, "plus")) {
        // Extract numbers from "What is X plus Y"
        int a = 0, b = 0;
        char* plus_pos = strstr(input_lower, "plus");
        if (plus_pos) {
            // Find numbers before and after "plus"
            for (int i = plus_pos - input_lower - 1; i >= 0; i--) {
                if (input_lower[i] >= '0' && input_lower[i] <= '9') {
                    a = atoi(&input_lower[i]);
                    break;
                } else if (strncmp(&input_lower[i], "two", 3) == 0) { a = 2; break; }
                else if (strncmp(&input_lower[i], "three", 5) == 0) { a = 3; break; }
                else if (strncmp(&input_lower[i], "four", 4) == 0) { a = 4; break; }
                else if (strncmp(&input_lower[i], "five", 4) == 0) { a = 5; break; }
            }
            
            for (int i = plus_pos - input_lower + 4; input_lower[i]; i++) {
                if (input_lower[i] >= '0' && input_lower[i] <= '9') {
                    b = atoi(&input_lower[i]);
                    break;
                } else if (strncmp(&input_lower[i], "two", 3) == 0) { b = 2; break; }
                else if (strncmp(&input_lower[i], "three", 5) == 0) { b = 3; break; }
                else if (strncmp(&input_lower[i], "four", 4) == 0) { b = 4; break; }
                else if (strncmp(&input_lower[i], "five", 4) == 0) { b = 5; break; }
            }
            
            if (a > 0 && b > 0) {
                result = call_int_int_function("add", a, b);
                if (result.success) {
                    snprintf(function_result, MAX_FUNCTION_RESULT, "%s", result.result);
    ...
    // Fibonacci pattern
    if (strstr(input_lower, "fibonacci") || strstr(input_lower, "comes after")) {
        if (strstr(input_lower, "after 5")) {
            result = call_int_function("fibonacci", 6);
            if (result.success) {
                snprintf(function_result, MAX_FUNCTION_RESULT, "%s", result.result);
                return function_result;
            }
    ...
    // Pi pattern
    if (strstr(input_lower, "what is pi")) {
        result = call_double_int_function("calculate_pi", 5);
        if (result.success) {
            snprintf(function_result, MAX_FUNCTION_RESULT, "%s", result.result);
            return function_result;
        }
    ...
    // Definition patterns
    if (strstr(input_lower, "what is a ") || strstr(input_lower, "what are ")) {
        char* word = NULL;
        if (strstr(input_lower, "what is a ")) {
            word = strstr(input_lower, "what is a ") + 10;
        } else if (strstr(input_lower, "what are ")) {
            word = strstr(input_lower, "what are ") + 9;
        }
        
        if (word) {
            // Extract the word (until space or end)
            char term[100] = {0};
            int i = 0;
            while (word[i] && word[i] != ' ' && word[i] != '?' && i < 99) {
                term[i] = word[i];
                i++;
            }
            term[i] = '\0';
            
            result = call_string_function("define", term);
            if (result.success) {
                snprintf(function_result, MAX_FUNCTION_RESULT, "%s", result.result);
                return function_result;
            }
    ...
    if (strstr(input_lower, "if") && strstr(input_lower, "is a") && strstr(input_lower, "what is")) {
        if (strstr(input_lower, "felix") && strstr(input_lower, "cat")) {
            result = call_string_function("classify", "felix");
            if (result.success) {
                snprintf(function_result, MAX_FUNCTION_RESULT, "%s", result.result);
                return function_result;
            }
    ...
    // Derivative pattern
    if (strstr(input_lower, "derivative") && strstr(input_lower, "squared")) {
        result = call_int_int_function("multiply", 2, 1);  // 2x for x^2 derivative
        if (result.success) {
            snprintf(function_result, MAX_FUNCTION_RESULT, "%sx", result.result);
            return function_result;
        }
    }
    
    return NULL;  // No function pattern matched
}

// Generate response with 100-token context
void generate_response(ChatSystem* sys, const char* input, char* output, int max_len) {
    // First, try to handle with function calls
    char* function_result = try_function_call(input);
    if (function_result) {
        strncpy(output, function_result, max_len - 1);
        output[max_len - 1] = '\0';
        return;
    }
    
    // If no function matched, use pattern matching
    // Tokenize input
...
void chat_loop(ChatSystem* sys) {
    ...
    printf("\n=== GAIA Chat V5 %s===\n", use_superposition ? "(Superposition Mode) " : "");
...
int main(int argc, char* argv[]) {
    printf("GAIA Chat System V5 - Logic Gates + Functions + Selective Superposition\n");
    printf("=====================================================================\n\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--superposition") == 0) {
            ...
            printf("Options:\n");
            printf("  --superposition       Enable selective superposition for ambiguous contexts\n");
            printf("  --debug-superposition Show superposition selection details\n");
    ...
    // Initialize function registry and register GAIA functions
    function_registry_init();
    register_gaia_functions();
    
    printf("Function system initialized\n");
    list_functions();
